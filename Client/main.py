...
class OtherPlayer: #멀티에서 다른 플레이어를 표시하기 위한 객체.
    def __init__(self, cx, cy, zx, zy, imagefolder, name): #이미지의 기본정보를 지정
        ...
        self.img = pygame.transform.scale(img, (self.sizeY*MAPTILESIZE//10 * len(name),self.sizeY*MAPTILESIZE//5))
    ...
    def display(self): #화면에 표시
        ...
        screen.blit(self.img, (self.coordX*MAPTILESIZE+ORIGINPOINT.x - (self.sizeY*MAPTILESIZE//10 * len(self.name)) // 2, self.coordY*MAPTILESIZE+ORIGINPOINT.y - self.sizeY*MAPTILESIZE // 2 - self.sizeY*MAPTILESIZE//5)) #텍스트 표시
        #닉네임 출력
